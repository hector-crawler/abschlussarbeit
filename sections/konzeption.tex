\section{Konzeption des leichten Krabbelroboters}
...
Die gewählte Ausführung bietet eine Auflösung von 500 Zählimpulsen pro Umdrehung (Counts per Revolution, CPR). Damit lässt sich selbst eine sehr feine Wegauflösung realisieren. Bei einem Raddurchmesser von \SI{5}{\centi\m} entspricht ein Impuls etwa \SI{0,31}{\milli\m} Bewegung – ausreichend, um auch kleinste Bewegungen des Roboters zu erfassen.
...
Da der alte Akku, ein \textit{XCell LiPo Cracker CAR} \cite{xcell_akku} mit zwei Zellen, \SI{7.4}{\volt} und \SI{5400}{\milli\ampere\hour}, durch sein hohes Gewicht von \SI{310}{\gram} ca. \SI{25}{\percent} des Gesamtgewichts des Roboters ausmachte, wurde ein leichterer Akku gesucht.

Der neu ausgewählte \textit{Gens ace Modellbau-Akkupack (LiPo)} \cite{gens_ace_akku} ist mit seinen ebenfalls \SI{7.4}{\volt} Ausgangsspannung ähnlich aufgebaut, wiegt allerdings nur noch \SI{66}{\gram} und ist deutlich kompakter. Aufgrund des geplanten Einsatzes als Demonstrationsroboter ist auch die damit verbundene geringere Laufzeit kein Problem.

\begin{align*}
t_{\text{Betrieb, alt}} &= \frac{\SI{5400}{\milli\ampere\hour}}{\SI{1070}{\milli\ampere}} \approx \SI{5}{\hour} \\
t_{\text{Betrieb, neu}} &= \frac{\SI{1500}{\milli\ampere\hour}}{\SI{1070}{\milli\ampere}} \approx \SI{1.4}{\hour}
\end{align*}

\SI{1070}{\milli\ampere} sind dabei der maximal gemessene Strom (bei der Bewegung der Motoren) und nicht der Normalzustand.
